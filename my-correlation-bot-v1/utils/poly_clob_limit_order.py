...
def main() -> None:
    parser = argparse.ArgumentParser(description="Preview or place a Polymarket CLOB limit order.")
    parser.add_argument("--token-id", required=True, help="CLOB token ID / asset ID from Gamma clobTokenIds.")
    parser.add_argument("--side", required=True, choices=["BUY", "SELL"], help="Order side.")
    parser.add_argument("--price", required=True, type=float, help="Limit price, e.g. 0.45.")
    parser.add_argument("--size", required=True, type=float, help="Outcome-token share size.")
    parser.add_argument("--type", default="GTC", choices=["GTC", "GTD"], help="Limit order type.")
